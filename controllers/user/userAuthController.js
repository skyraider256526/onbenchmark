    ...
    if(!User){
      ...
      if (resetPassword) {
        const newUser = await User.update(
          { firstName, lastName, companyName, email, password, mobileNumber, modifiedBy },
          { transaction: t }
        )
        console.log(newUser)
      } else {
...
exports.getListOfClients = async (req, res, next) => {
  const user = await models.user.findAll({ where: { isActive: true } });
  const roles = await models.role.findAll({where:{roleName:'client',isActive:true}})
  const userFound = roles.map( async (data) => await models.user.findOne({where:{id:data.role_id, isActive:true}}))
...
exports.getListOfResourceManager = async (req, res, next) => {
  const user = await models.user.findAll({ where: { isActive: true } });
  const roles = await models.role.findAll({where:{roleName:'resource_manager',isActive:true}})
  const userFound = roles.map( async (data) => await models.user.findOne({where:{id:data.role_id}}))
  const resource_managers = await Promise.all(userFound)
  // console.log("roles ", uf)
  if (user.length > 0) { 
    return res
      .status(200)
      .json({ message: "all users detailed fetched successfully", resource_managers });
  } else {
    return res.status(200).json({ message: "no users found" });
  }
};