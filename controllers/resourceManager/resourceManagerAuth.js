...
exports.updateResourceManager = async (req, res, next) => {
    let {
      ...
      await sequelize.transaction(async (t) => {
        const resourceManager_user = await models.resourceManager.findOne({ where: { id } });
...
exports.getAllResourceManagers = async (req, res, next) => {
    const user = await models.resourceManager.findAll({where: { isActive:true }})
    console.log(user)
    if(user.length > 0){
        return res.status(200).json({message:'all resource manager detailed fetched successfully', user})
    }else{
...
exports.getResourceManagerById = async (req, res, next) => {
  ...
  if (userResult) {
    return res
      .status(200)
      .json({ message: "resource manager detail fetch successfully", userResult });
  } else {
