...
exports.userLogin = async (req, res, next) => {
  ...
  if (!checkUser) {
    res
      .status(401)
      .json({ message: "you have entered incorrect username or password" });
  } else {
    let userDetail = await checkUser.comparePassword(password);
    console.log("user detail ", userDetail);
    if (userDetail === true) {
      console.log("check role of user ", checkUser.dataValues.id);
      let checkRole = await models.role.findOne({
        where: { role_id: checkUser.dataValues.id },
      });
      console.log("check role ", checkRole.roleName);
